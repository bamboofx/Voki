...
{	
	...
	{	
		...
		public function ShowController($show:SlideShowStruct, $bgcolor:Number = 0xffffff):void
		{
			...
			bg_color = (isNaN($bgcolor)) ? -1 : $bgcolor;
			if ($show.bg_color != null) bg_color = ( isNaN(parseInt($show.bg_color)) ) ? -1 : parseInt($show.bg_color);
			...
			if (assets_to_load == 0) { // assetLoaded gets called later on by bg holder if setBackground/loadBG is called, thus assets_to_load will be < 0 and in this case you should not continue as is the case with the initial loading of assets (only when assets_to_load == 0 should you continue) maybe a better solution is to use a different event handler for bgs loaded afer initial setup so this same method doesn't get called and rely on such specific logic/treatment of the int value assets_to_load
				...
				function onBadWordsLoaded():void
				{
					if (loading_status == STATUS_PRELOADING) {
						...
						bg_holder.displayAsset(scene_data.bg);// : bg_holder. ;
						bg_holder.scaleX = scene_data.backgroundTransform.scaleX;// scene_data.bg_scale
		...
		private function resume():void
		{
			bg_holder.resume();
			host_holder.resume();
			if (is_speaking || bg_holder.getStatus() == "paused") skin_holder.activatePauseButton();
		}
		...
		private function pause():void
		{
			bg_holder.pause();
			host_holder.freeze();
			if (is_speaking || bg_holder.getStatus() == "playing") skin_holder.activatePlayButton();
		}
		...
		public function preloadScene($num:int):void
		{
			if ($num >= 0 && $num <= show_data.scenes.length-1 && loading_status == STATUS_LOADED)
			{
				...
				if (t_scene_data.bg != null) loadBg(BackgroundStruct(t_scene_data.bg));
		...
		public function loadBg($bs:BackgroundStruct, $display:Boolean = false, transform:Object = null):void
		{
			...
			if (transform) {
				bg_holder.addEventListener(AssetEvent.ASSET_INIT, function onInit(event:AssetEvent):void
				{
		...
		{
			scene_data.backgroundTransform = transform;
			var trans:Object = scene_data.backgroundTransform; // need to copy from scene_data_backgroundTransform in case transform doesn't have all props
			bg_holder.x = trans.x;
			bg_holder.y = trans.y;
			bg_holder.rotation = trans.rotation;
			bg_holder.scaleX = trans.scaleX;
			bg_holder.scaleY = trans.scaleY;
		}
		...
		{
			bg_holder.displayBgFrame(offset);
		}
		...
		*/
		...
		{
			//----trace("SHOW CONTROLLER ---- bg ready -- "+$ev.data.toString());
			var t_bs:BackgroundStruct = BackgroundStruct($ev.data);
			loadBg(t_bs, true);
		}
		...
		private function e_displayImmediately($ev:AssetEvent):void
		{
			...
			{
				var t_bs:BackgroundStruct = BackgroundStruct($ev.data)
				bg_holder.displayAsset(t_bs);
				bg_holder.scaleX = bg_holder.scaleY = (t_bs.type == "video") ? Constants.FLV_SCALE_DEFAULT : Constants.BG_SCALE_DEFAULT;
		...
		private function e_talkStarted($ev:VHSSEvent):void
		{
			...
			{
				var event_str:String = ((hack_audio_request.engine.length < 2) ? "0"+ hack_audio_request.engine : hack_audio_request.engine) + ((hack_audio_request.lang.length < 2) ? "0"+ hack_audio_request.lang : hack_audio_request.lang) + ((hack_audio_request.voice.length < 2) ? "0"+ hack_audio_request.voice : hack_audio_request.voice);
				hack_audio_request = null;
				VHSSPlayerTracker.event("aptts", scene_data.id, 0, event_str);
			}
			else
			{
		...
		{
			//trace("ShowController ++++ TALK ENDED EVENT HANDLER  status:: "+bg_holder.getStatus());
			is_speaking = false;
		...
		public function destroy():void
		{
			...
			if (bg_holder != null)
			{
				//trace("Show - destroy - bg_holder --- NO DESTROY");
				bg_holder.stop();
