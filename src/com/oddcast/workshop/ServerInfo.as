...
//		public static var throttle_microphone_max_count					:int		= NO_VALUE_NUM;
		...
		{		mid 			= null;
				...
				if (!isEmptyString(swfInfo.parameters.mId)) 		mid = swfInfo.parameters.mId;
				else if (!isEmptyString(swfInfo.parameters.mid)) 	mid = swfInfo.parameters.mid;
					
				if (mid != null && mid.indexOf(".") >= 0) 
				{	var midArr:Array = mid.split(".");
					mid = midArr[0];
					viralSourceId = parseInt(midArr[1]);
					if (viralSourceId == 1) isEmbedSession = true; //1=embed
					else if (viralSourceId == 2) isEmailSession == true; //2=email
					//3 = get URL
					//4 = workshop opened from player link
					//5 = gallery
				}
