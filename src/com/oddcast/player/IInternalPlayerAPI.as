...
package com.oddcast.player
{
	...
	public interface IInternalPlayerAPI extends IPublicPlayerAPI
	{
		...
		/**
		 * Load a background in the current scene with the option of setting a transform Matrix. Loaded background will only apply to the immediate state of the
		 * scene. Navigating away from the scene and back again will result in the background returning to it's original state.
		 * 
		 * @param $bg <code>BackgroundStruct</code> of the background to apply. Passing <code>null</code> will remove the active background
		 * @param transform <code>Object</code> to apply to the background once it has loaded. Any or all of the following properties may be used: x,y,rotation,scaleX, scaleY. Passing null will not modify the background's transform.
		 * 			example: { x:40, y:50, rotation:35, scaleX:1.2, scaleY:1.6 }
		 * 
		 * @event bg_loaded:VHSSEvent - background has loaded. 
		 */
		function loadBackgroundWithTransform(background:BackgroundStruct, transform:Object):void;
		/**
		 * Load a host. Replaces the currenty loaded host. If switch 2d to 3d or 3d to 2d you should also get a new reference to the
